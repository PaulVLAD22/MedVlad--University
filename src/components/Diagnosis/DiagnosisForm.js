...
const DiagnosisForm = () => {
  ...
  return (
    ...
    >
              ...
              .map((symptom) => {
                if (symptom.name.includes(searchWord))
                  return (
                    <SymptomLabel
                      selectSymptom={() => {
                        selectSymptom(symptom.originalIndex);
                      }}
                      key={symptom.id}
                      name={symptom.name}
                      selected={selectedSymptoms[symptom.originalIndex] == 1}
                    />
                  );
              })}
          </Flex>
      ...
      <Button width="auto" px="2" py="1" onClick={resetSelectedSymptoms} mb="3">
        {" "}
        Reset Symptomps
      </Button>
      <Button width="auto" px="2" py="1" onClick={calculateCondition}>
        {" "}
        Calculate Condition
      </Button>
